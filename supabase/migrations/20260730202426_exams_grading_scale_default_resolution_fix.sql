...
-- Design fix: schools.default_grading_scale_id and grading_scales.is_default were two sources of
-- truth for the same fact. Keep one: grading_scales.is_default (already unique-indexed per school),
-- drop the redundant column, and have resolution look it up directly.
alter table schools drop column default_grading_scale_id;
