...
begin
  ...
  -- Lock the application row for the duration of this transaction so that
  -- two concurrent "Complete Enrollment" calls for the same application
  -- are serialized: the second call blocks here until the first commits,
  -- then re-reads the now-'enrolled' status and takes the idempotent
  -- early-return path below instead of re-running enrollment.
