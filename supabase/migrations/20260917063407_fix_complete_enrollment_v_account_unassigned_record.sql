-- Fixes a known, already-flagged gap in complete_enrollment(): v_account is only ever
-- populated inside `if auth_has_permission('finance.write') then ... end if`. When that
-- permission is absent, v_account is never assigned, and the later unconditional reference to
-- v_account.payment_reference in the admission_enrollment_history insert raises "record
-- \"v_account\" is not assigned yet" -- aborting the entire (transactional) enrollment for any
-- staff member completing it without finance.write. See
-- 20260823041527_complete_enrollment_reconcile_stk_payments.sql's header comment, where this
-- was first documented and deliberately left unfixed ("flagged for Lucy separately").
--
-- Fix: capture the one field actually needed (payment_reference) into a plain text variable
-- that starts out NULL and is only set when v_account itself gets populated, instead of
-- referencing the possibly-unassigned record directly. No other behavior changes.
...
begin
  ...
  if auth_has_permission('finance.write') then
    begin
      ...
      -- Only ever touch v_account here, inside the branch that actually populates it -- see
      -- the migration header comment for why the later insert uses v_payment_reference instead.
      v_payment_reference := v_account.payment_reference;
  ...
  select v_app.school_id, p_application_id, v_app.resulting_student_id, v_actor, s.admission_number, v_invoice_id, v_payment_reference
