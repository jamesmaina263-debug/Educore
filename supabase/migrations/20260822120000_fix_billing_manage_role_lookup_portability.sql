...
-- looks the role up by name instead. Zero-behavior-change on the live database: the row
-- already exists with the correct role_id, so the unique partial index (role_id,
-- permission_key) WHERE school_id IS NULL makes this a no-op there while fixing
-- portability going forward. Applied live via Supabase on 2026-08-22; this file brings the
-- migration history back in sync with prod.
