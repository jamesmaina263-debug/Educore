-- Phase 8, Item 18: "All financial actions ... are audited with user, timestamp, action, record,
-- and relevant status change." Payment creation/allocation/reversal already write to audit_log
-- (this phase's earlier migrations). Discount/expense/waiver approvals existed before this phase
-- but never wrote an audit trail — extending them here, same audit_log table, no second log.
