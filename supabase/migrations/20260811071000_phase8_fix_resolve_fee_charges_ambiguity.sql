-- ============================================================
-- Fix, found via live testing: resolve_fee_charges_for_student (Phase 8)
-- has declared a returns-table OUT column named fee_category, which
-- collides with the unqualified fee_structures.fee_category column
-- referenced in its own WHERE clauses -- Postgres reports this as
-- ambiguous rather than silently picking one, so every call has been
-- failing since Phase 8, not just from the Phase 12 wizard's Finance
-- step preview that surfaced it. generate_invoices and
-- create_or_get_invoice_for_student don't have this bug (no OUT
-- parameter of that name), so this is the only function that needs it.
-- Logic is unchanged -- just qualifying fee_structures.fee_category.
-- ============================================================
