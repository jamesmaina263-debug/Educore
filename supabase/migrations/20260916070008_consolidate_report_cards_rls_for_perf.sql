-- Part 14. report_cards had 2 UPDATE-only permissive policies, each already
-- with an explicit with_check identical to its own qual, so the combined
-- with_check is the same expression as the combined using. Applied directly
