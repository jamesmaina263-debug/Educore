...
| `RESEND_API_KEY` / `RESEND_FROM_ADDRESS` | Supabase Edge Function secrets | Email | Set and confirmed working — `RESEND_FROM_ADDRESS` is a verified custom-domain address (educoreafrica.com is live, see MARKETING_SITE_STATUS.md). Re-verified 2026-09-14: a live test through the deployed `notify-platform-admin` function delivered to a genuinely external address (Resend returned a real 200), and separately, `EmailProvider.send()` sending from `james.maina@educoreafrica.com` (a different mailbox on the same verified domain) for the demo-request-assignment notification also succeeded — confirming the domain-level verification, not just the one specific address. The previous note here (2026-08-22) said this was still Resend's shared sandbox sender `onboarding@resend.dev`, restricted to delivering only to the Resend account's own email; that has since been resolved. |
