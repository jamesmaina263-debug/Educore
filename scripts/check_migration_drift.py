...
history (supabase_migrations.schema_migrations), so drift (a migration applied
to prod under one timestamp/name, but committed to the repo under a different,
invented one -- or never committed at all) gets caught in CI instead of
discovered weeks later. This repo has hit this exact pattern repeatedly.
...
def main():
    ...
    if missing:
        ...
        print("If a version genuinely has no content to restore (e.g. a "
              ...
              "because it's inconvenient to reconstruct. If a version has content "
              "but was reconstructed via live schema introspection rather than the "
              "original recorded statements, double check it against "
              "supabase_migrations.schema_migrations directly -- introspection can "
              "misattribute a *later* modification of an object to an *earlier* "
              "migration's version if the object was changed again since.")
        print()
