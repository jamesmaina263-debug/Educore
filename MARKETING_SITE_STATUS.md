...
**Live URL:** production is `educoreafrica.com` (custom domain purchased and attached to the Vercel project; `src/lib/site.ts` updated accordingly on 2026-08-30).
...
- **Site-wide SEO:** new `src/lib/site.ts` (`SITE_URL` was originally `https://educore-beige.vercel.app`, the project's Vercel-assigned production alias — confirmed via `Vercel:get_project`; there was no purchased custom domain at the time, so this wasn't invented. **Update, 2026-08-30:** the custom domain `educoreafrica.com` has since gone live and `SITE_URL` was swapped to it in this same file, exactly as anticipated here). `metadataBase` + default Open Graph/Twitter tags added to the root layout. Every marketing page's `metadata` export — the original 5 from Phases 3–7, plus `/about` and `/contact` once Phase 8 landed mid-session — got Open Graph/Twitter/canonical metadata added; the homepage got an explicit `metadata` export for the first time (it had been silently inheriting the generic root-layout fallback).
