...
- **Not yet done as part of this phase:** live Vercel preview-deployment fetch for `/faq` (and to re-confirm `/robots.txt` / `/sitemap.xml` / `/about` / `/contact` serve correctly on the actual preview post-rebase). Do this before considering Phase 9 fully closed, same as every prior phase's verification bar.
...
**For the next session, in one sentence:** Phases 1–9 (routing fix, design system, homepage, platform, solutions, AI & automation, pricing, about/contact, FAQ & SEO) are all done and verified on branch `feature/marketing-site` (Phase 9's live-preview fetch is the one open item); start at Phase 10 (final UX & quality audit) once that's confirmed, follow the ground rules in Section 5, and do not merge to `main` until Phase 10 is explicitly signed off by the owner.
