...
import { DashboardSection, type DashboardStats } from "@/components/boarding/dashboard-section";
import { StructureSection, type HouseRow, type StaffOption } from "@/components/boarding/structure-section";
import { AllocationSection, type AllocationRow, type StudentOption, type AvailableBedOption } from "@/components/boarding/allocation-section";
import { RollCallSection, type BoardingStudentRow } from "@/components/boarding/roll-call-section";
import { TransfersSection, type TransferRow } from "@/components/boarding/transfers-section";
import { IncidentsSection, type IncidentRow } from "@/components/boarding/incidents-section";
import { ReportsSection, type ReportsData } from "@/components/boarding/reports-section";
...
export default async function BoardingPage({
  ...
  searchParams: Promise<{ tab?: string; date?: string; session?: string }>;
}) {
  const { tab, date, session } = await searchParams;
  ...
  // House master/assistant names need a second lookup since Supabase can't
  // embed two FKs to the same table (school_users) cleanly in one query for houses.
  ...
  // Assemble the nested House -> Dormitory -> Room -> Bed tree.
  ...
  // Roll call data for the selected date/session (defaults to today, AM).
  ...
  const rollCallStudents: BoardingStudentRow[] = allocations
    ...
    .filter((s) => s.stream_id); // a boarding student always has a class per Section 5, but guard defensively
  ...
  return (
    <AppShell
      breadcrumbs={[{ label: schoolName ?? "EduCore", href: "/dashboard" }, { label: "Boarding" }]}
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {canReadAny ? "Houses, dormitories, rooms, beds, and boarding operations." : "Your child's boarding allocation."}
          </p>
        </div>
        ...
        <Tabs defaultValue={tab ?? "dashboard"}>
          ...
          <TabsContent value="structure">
            <StructureSection houses={houseTree} staff={staffOptions} canWrite={canWrite} />
          </TabsContent>
          <TabsContent value="allocation">
            <AllocationSection
              allocations={allocationTableRows}
          ...
          <TabsContent value="rollcall">
            <RollCallSection date={rollCallDate} session={rollCallSession} students={rollCallStudents} canWrite={canWrite} />
          </TabsContent>
          <TabsContent value="transfers">
            <TransfersSection
              transfers={transferTableRows}
              boardingStudents={boardingStudentOptions}
              availableBeds={availableBeds}
              canWrite={canWrite}
            />
          </TabsContent>
          <TabsContent value="incidents">
            <IncidentsSection incidents={incidentTableRows} boardingStudents={boardingStudentOptions} canWrite={canWrite} />
          </TabsContent>
